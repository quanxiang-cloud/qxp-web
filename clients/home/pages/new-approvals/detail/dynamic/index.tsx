...
import { getAllProcessInfo } from '../../api';
import { getAllPipelineProcess } from '../../util';
...
export default function ProcessHistory(props: Props) {
  const { processInstanceID, taskType } = useParams<{ processInstanceID: string, taskType: string}>();
  ...
  const {
    isLoading, data: _data = [], isError,
  } = useQuery<any, Error>(
    [processInstanceID, 'GET_PROCESS_HISTORIES'],
    () => {
      getAllProcessInfo(processInstanceID)
        .then((res: any)=>{
