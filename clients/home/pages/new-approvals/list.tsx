...
import { getPipelineTodoApprovalCount, getPipelineTodoFillInCount } from './api';
...
function Approvals(): JSX.Element {
  ...
  useQuery(['flow-instance-count', listType], async () => {
    return await getPipelineTodoApprovalCount();
  }, {
    onSuccess: (data) => setApprovelCount(data?.total || 0),
  });

  useQuery(['fill-in-count', listType], async () => {
    return await getPipelineTodoFillInCount();
  }, {
    onSuccess: (data) => setFillCount(data?.total || 0),
  });

  useEffect(()=>{
    approvelCount && setCountMap(approvelCount);
  }, [JSON.stringify(approvelCount), fillCount]);
