...
function CreateViewModal({
  ...
  const [_viewType, _setViewType] = useState<ViewType>(ViewType.SchemaView);
  ...
  const excludeViewNames =
    useMemo(() => {
      if (modalType === 'editView') {
        return views.filter((view) => view.name !== viewParams?.name).map((view) => view.name);
      }
    }, [modalType, views]) || [];
      ...
      {!viewType && isCreateView ? (
        ...
        >
          ...
          {viewType === ViewType.StaticView && isCreateView && (
            <Item required name="fileUrl" label="上传静态页面">
              <StaticViewUpload />
            </Item>
          )}
