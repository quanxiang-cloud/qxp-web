...
import type { ProcessVariableAssignmentData } from '@newFlow/content/editor/type';
...
import { getVariableList } from '@portal/modules/new-work-flow/api';
...
export default function AssignmentConfig({ defaultValue, onSubmit, onCancel }: Props): JSX.Element {
  ...
  const { data: variables, isLoading } = useQuery<ProcessVariable[]>(['FETCH_PROCESS_VARIABLES'], () => {
    ...
    return getVariableList(flowID).then((res: any) => {
      const { variable = [] } = res || [];
      return variable?.filter(({ type }: any) => type !== 'SYSTEM');
    });
  });
