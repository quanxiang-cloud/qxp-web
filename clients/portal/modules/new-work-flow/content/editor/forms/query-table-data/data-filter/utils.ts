...
export function getOperators(type: string, enums: any[] | undefined): LabelValue[] {
  ...
  case 'number':
    return [...OPERATORS_NUMBER, ...OPERATORS_ARRAY_MULTIPLE];
  case 'datetime':
    return [...OPERATORS_DATE, ...OPERATORS_ARRAY_MULTIPLE];
  case 'boolean':
    return [...OPERATORS_BOOL, ...OPERATORS_ARRAY_MULTIPLE];
  case 'label-value':
    return [...OPERATORS_LABEL_VALUE, ...OPERATORS_ARRAY_MULTIPLE];
  default:
    if (enums && enums.length) {
      return OPERATORS_ARRAY_MULTIPLE;
    }

    return [...OPERATORS_STRING, ...OPERATORS_ARRAY_MULTIPLE];
  }
}
...
export function setValueFormCondition({ valueFrom, key, op, value, schema, outputKey, outputNodeID }: ValueFromProps | any): any {
  if (valueFrom === 'form') {
    return {
      key,
      op,
