...
export const Menu = ({ menus, visible }: IMenus) => {
  ...
  return (
    <div className="w-full h-full z-10 bg-black bg-opacity-50 fixed hidden transition duration-200" ref={maskRef}>
      <div className={classnames("-left-full transform p-8 max-w-full w-2-9-dot-4 absolute bottom-0 top-0 flex flex-col", useCss({
        'background': '#F0F6FF',
        ...
        <List
          ...
          itemClassName="pb-dot-8"
          items={
            menus.map(({ title, desc, iconClassName, iconUrl, address }) => (
              <div className="flex flex-row justify-between items-center bg-white px-4 py-dot-8 rounded">
                <ItemWithTitleDesc  
                  title={title}
                  desc={desc}
                  itemRender={() => (
                    <div className={classnames("p-dot-3-6 rounded-lg rounded-tr-none leading-4", iconClassName)}>
                ...
                />
                <Link to={address}>
                  <Icon name="next" type="dark" size={24} />
                </Link>
              </div>
            ))
          }
        />
        <div className={classnames("self-start cursor-pointer mt-8 flex flex-between items-center rounded-lg rounded-tr-none px-dot-8 py-1", useCss({
