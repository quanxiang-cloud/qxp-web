...
export default class SelectTree extends React.Component<Props> {
  ...
  render() {
    ...
    return (
      <Control id="tree-select-box" tabIndex="0" onBlur={this.handleBlur} name={name} style={styles.box} className='select'>
        <div onClick={() => this.setState({ showSelectBox: !showSelectBox })} className="select-control">
          ...
          <span className="select-arrow-zone">
            <span className="select-arrow">
              {!!selectValue && (
                <Icon onClick={this.handleClear} changeable clickable name="close" type="dark" size="small" />
              )}
            </span>
          </span>
        </div>
