...
import { ItemWithTitleDesc } from './Item-with-title-desc2';
  ...
  return (
    <header className={twCascade('flex justify-between items-center pb-4 opacity-95', className)}>
      <ItemWithTitleDesc
        ...
        descClassName={twCascade('transition ease-linear text-1-dot-2 text-697886', descClassName)}
      ...
      />
      {isString(action) && (
        <a className={twCascade('transition ease-linear text-1-dot-4 underline text-324558')}>
          {action}
        </a>
      )}
